            ...
            {showUserMenu && (
              <>
                ...
                <div className="absolute right-0 top-[110%] z-50 w-64 rounded-3xl border border-black/5 bg-white/95 p-2 shadow-[0_20px_60px_rgba(0,0,0,0.12)] backdrop-blur-xl text-left flex flex-col gap-1 animate-in fade-in zoom-in-95 duration-200 dark:border-white/10 dark:bg-slate-900/95">
                  ...
                  <button type="button" onClick={() => { setShowUserMenu(false); handleOpenSettings() }} className="flex items-center gap-3 w-full rounded-xl px-3 py-2.5 text-[13px] font-semibold text-slate-700 hover:bg-slate-100 hover:text-slate-900 transition-colors dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white">
        ...
        {showUserMenu && (
          <>
            ...
            <div className="absolute right-0 top-[110%] z-50 w-64 rounded-3xl border border-black/5 bg-white/95 p-2 shadow-[0_20px_60px_rgba(0,0,0,0.12)] backdrop-blur-xl text-left flex flex-col gap-1 animate-in fade-in zoom-in-95 duration-200 dark:border-white/10 dark:bg-slate-900/95">
              ...
              <button
                type="button"
                onClick={() => { setShowUserMenu(false); handleOpenSettings() }}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-md animate-fade-in">
          <div className="w-full max-w-lg rounded-3xl border border-white/50 dark:border-white/10 bg-white/95 dark:bg-slate-900/95 p-6 shadow-[0_34px_100px_rgba(15,23,42,0.25)] backdrop-blur-xl animate-vault-morph">
            ...
            <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:justify-end">
              {hasUnsyncedChanges ? (
                <button type="button" onClick={() => { setLockModalOpen(false); handleOpenSettings() }} className="min-h-11 rounded-xl border border-red-200 bg-red-50 px-4 text-sm font-semibold text-red-800 transition-colors hover:bg-red-100">
                  Descargar copia local (Cifrada/CSV) para no perder datos
                </button>
              ) : (
                <button type="button" onClick={() => { setLockModalOpen(false); handleOpenSettings() }} className="min-h-11 rounded-xl border border-amber-200 bg-amber-50 px-4 text-sm font-semibold text-amber-800 transition-colors hover:bg-amber-100">
    ...
    return (
      <div className="vault-shell vault-stage fixed inset-0 flex h-dvh max-h-dvh flex-col overflow-hidden overscroll-none">
        ...
        <nav className="vault-mobile-dock" aria-label="Navegación principal">
          ...
          <button type="button" title="Ajustes" className={`vault-dock-item ${settingsOpen ? 'vault-button-primary text-white' : ''}`} onClick={() => { setInboxModalOpen(false); handleOpenSettings() }} aria-current={settingsOpen ? 'page' : undefined}>
          ...
          <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-md animate-fade-in">
            <div className="w-full max-w-lg rounded-3xl border border-white/50 dark:border-white/10 bg-white/95 dark:bg-slate-900/95 p-6 shadow-[0_34px_100px_rgba(15,23,42,0.25)] backdrop-blur-xl animate-vault-morph">
              ...
              <div className="mt-6 flex flex-col gap-2">
                {hasUnsyncedChanges ? (
                  <button type="button" onClick={() => { setLockModalOpen(false); setFocusCsvExport(true); handleOpenSettings() }} className="min-h-12 rounded-xl border border-red-200 bg-red-50 px-4 text-sm font-semibold text-red-800">
                    Descargar copia local (Cifrada/CSV) para no perder datos
                  </button>
                ) : (
                  <button type="button" onClick={() => { setLockModalOpen(false); setFocusCsvExport(true); handleOpenSettings() }} className="min-h-12 rounded-xl border border-amber-200 bg-amber-50 px-4 text-sm font-semibold text-amber-800">
                    Descargar copia de seguridad (CSV) por si acaso
                  </button>
                )}
