          ...
          <div className="space-y-5 overflow-y-auto max-h-[360px] pr-1 scrollbar-thin">
            ...
            <section className="space-y-2.5">
              ...
              <form onSubmit={handleExport} className="space-y-2">
                ...
                />
                {exportError && <p className="text-[10px] text-red-600 font-semibold">{exportError}</p>}
                {exportSuccess && <p className="text-[10px] text-green-600 font-semibold">{exportSuccess}</p>}
                <button
                  type="submit"
                  disabled={loadingExport || !exportPassword}
                  className="w-full rounded-xl bg-text-primary py-2.5 text-xs font-semibold text-white hover:opacity-90 transition-opacity disabled:opacity-40 active:scale-[0.98] transition-transform"
                >
            ...
            <section className="space-y-2.5">
              ...
              <form onSubmit={handleImport} className="space-y-3">
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-border-subtle bg-surface px-3 py-2.5 text-xs text-text-primary placeholder:text-text-tertiary outline-none focus:border-border transition-colors font-medium"
                />
                {importError && <p className="text-[10px] text-red-600 font-semibold">{importError}</p>}
                {importSuccess && <p className="text-[10px] text-green-600 font-semibold">{importSuccess}</p>}
                <button
                  type="submit"
                  disabled={loadingImport || !backupFile || !importPassword}
                  className="w-full rounded-xl border border-border bg-surface-elevated hover:bg-surface-hover py-2.5 text-xs font-semibold text-text-primary transition-colors disabled:opacity-50 active:scale-[0.98] transition-transform"
                >
          ...
          <div className="space-y-4 animate-fade-in">
            {cloudUserEmail === null ? (
              ...
              <form onSubmit={handleCloudAuth} className="space-y-4">
                ...
                {cloudActionError && <p className="text-[10px] text-red-600 font-semibold">{cloudActionError}</p>}
                {cloudError && <p className="text-[10px] text-red-600 font-semibold">{cloudError}</p>}

                <div className="flex flex-col gap-2">
                  <button
                    type="submit"
                    disabled={loadingCloudAction || !cloudEmail.trim() || cloudPassword.length < 6}
                    className="w-full rounded-xl bg-text-primary py-2.5 text-xs font-semibold text-white hover:opacity-90 disabled:opacity-40 active:scale-[0.98] transition-transform"
                  >
                    {loadingCloudAction
